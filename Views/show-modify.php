...
<div class="text-center mt-5 mb-3">
    ...
    <button type="submit" class="btn btn-secondary bg-danger text-black mt-3" value="back"
        onclick="window.location.href='<?php echo FRONT_ROOT?>Show/manageShows'"> Back</button>
</div>
    ...
    <div class="card card-body m-1">
        <form class="offset-md-1" action="<?php echo FRONT_ROOT ?>Show/modifyShow" method="POST">
            <div class="form-group row">
                <div class="col-sm-10">
                    <input type="hidden" class="form-control" name="idShow"
                        value=<?php echo $currentShow->getIdShow() ?>>
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <select class="custom-select" name="idRoom" id="idRoom">
                        ...
                        ?>
                        <option <?php if($room->getId() == $currentShow->getRoom()->getId()) echo "selected" ?>
                            value="<?php echo $room->getId()?>"><?php echo $room->getName()?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <select class="custom-select" name="idMovie" id="idMovie">
                        ...
                        ?>
                        <option
                            <?php if($movie->getMovieID() == $currentShow->getMovie()->getMovieID()) echo "selected" ?>
                            value="<?php echo $movie->getMovieID()?>"><?php echo $movie->getTitle()?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            <div class="form-group row ">
                ...
                <div class="col-sm-10">
                    <input type="date" class="form-control" name="date"
                        min="<?php echo $oneDayAhead->format("Y-m-d");?>" value=<?php echo $currentShow->getDate() ?>
                        required>
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="time" class="form-control" name="start" value=<?php echo $currentShow->getStart() ?>
                        required>
                </div>
            </div>
            <!--<button type="submit" name="button" class="btn btn-secondary bg-danger text-black col-2  float-right" >Send</button> -->
            <button type="submit" name="idShow"
                class="btn-resp-general btn btn-secondary bg-danger text-black col-2  float-right"
                value="<?php echo $currentShow->getIdShow()?>">Send</button>
        </form>
    </div>
</div>
<br>