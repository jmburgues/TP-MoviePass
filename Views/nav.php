				...
				<form class="form-inline my-2 my-lg-2 " action="<?php echo FRONT_ROOT?>Show/listByGenre" method=POST>
					...
					<div class="dropdown-menu " aria-labelledby="dropdownMenuButton">
						<?php if(isset($genresList) && $genresList != null) { 
								...
								<button type ="submit" class="dropdown-item btn btn-danger" name="genreId" value="<?php echo $genre->getId(); ?>"><?php echo $genre->getName();?></button>
								<?php 
								} 
							}?>
					</div>
				...
				<form class="form-inline my-2  my-lg-2 " action="<?php echo FRONT_ROOT?>Show/getMoviesByDate" method=POST?>
					...
					<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
						<?php  if(isset($moviesYearList) && $moviesYearList != null) { foreach($moviesYearList as $year){ ?>
							<button type ="submit" class="dropdown-item btn btn-danger" name="year" value="<?php echo $year;?>"><?php echo $year;?></button>
						<?php } } ?>
					</div>
				</form>
			</div>
		...
		<div>
			<div class="dropdown">
				<form class="form-inline my-2  my-lg-2 ">
					...
					<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
						<button type ="button" class="dropdown-item btn btn-danger" value="Sing In" onclick="window.location.href='<?php echo FRONT_ROOT?>User/showLoginForm'">Sing In</button>
						<button type ="button" class="dropdown-item btn btn-danger" value="Register" onclick="window.location.href='<?php echo FRONT_ROOT?>User/register'">Register</button>
					</div>
					<?php } else { ?> <!--IS LOGGED -->
					...
					<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
						<button type ="button" class="dropdown-item btn btn-danger" value="<?php echo $_SESSION['loggedUser']?>" name="userName" onclick="window.location.href='<?php echo FRONT_ROOT?>User/userView'">My Account</button>				

						<?php if($_SESSION['role'] == 'admin') { ?> <!-- IS ADMIN -->
								<button type ="button" class="dropdown-item btn btn-danger" value="AdminTools" onclick="window.location.href='<?php echo FRONT_ROOT?>User/adminView'">Admin Tools</button>
						<?php } else if($_SESSION['role'] == 'owner'){?>
							<button type ="button" class="dropdown-item btn btn-danger" value="AdminTools" onclick="window.location.href='<?php echo FRONT_ROOT?>User/adminView'">Admin Tools</button>
							<button type ="button" class="dropdown-item btn btn-danger" value="OwnerTools" onclick="window.location.href='<?php echo FRONT_ROOT?>User/ownerView'">Owner Tools</button>
							<?php } ?>
						<button type ="button" class="dropdown-item btn btn-danger" value="Logout" onclick="window.location.href='<?php echo FRONT_ROOT?>User/logout'">Logout</button>
					</div>
					<?php }?>
				</form>
			</div>
